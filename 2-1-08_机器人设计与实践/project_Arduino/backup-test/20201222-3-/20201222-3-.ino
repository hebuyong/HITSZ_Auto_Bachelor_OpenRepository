...
/*
  ...
  if ( digitalRead(R3) == HIGH)//直角右转
  {
    target1 = 0.3 * V;
    target2 = -0.3*V;
  }
  if (digitalRead(L3) == HIGH) //直角左转
  {
    target1 = -0.3*V;
    target2 = 0.3 * V;
  }
...
void control_lr(void)
{
  ...
  if (sum > 2)
  {
    ...
    if (sensor[1] + sensor[2] + sensor[3] >= 2)
    {
      target1 = -0.3 * V; target2 = V * 0.3;
    }
    if ((sensor[5] + sensor[6] + sensor[7] >= 2))
    {
      target1 = V * 0.3; target2 = -0.3 * V;
    }
  ...
  else
  {
    ...
    else
    {
      if (sum == 1)
      {
        ...
        {
          target1 = 0.8 * V; target2 = 1 * V;
        }
        else if (sensor[5])
        {
          target1 = 1 * V; target2 = 0.8 * V;
        }
        else if (sensor[2])
        {
          target1 = 0.3* V; target2 = 0.8 * V;
        }
        else if (sensor[6])
        {
          target1 = 0.8 * V; target2 = 0.3 * V;
        }
        else if (sensor[1])
        {
          target1 = 0 * V; target2 = 0.6 * V;
        }
        else if (sensor[7])
        {
          target1 = 0.6 * V; target2 = 0 * V;
        }
      }
      if(sum==2)
      {

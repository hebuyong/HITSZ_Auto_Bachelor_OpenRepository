...
void View1()
{
	...
	loadimage(&img_backgroundbrick, "\\推箱子素材\\backgroundbrick.jpg");
	loadimage(&img_box, "\\推箱子素材\\box.jpg", 2.5 * SIZE, 2.5 * SIZE);
...
void View2()
{
	...
	loadimage(&img_backgroundbrick, "\\推箱子素材\\backgroundbrick.jpg");
	loadimage(&img_esc1, "\\推箱子素材\\esc_1.jpg");
	loadimage(&img_esc2, "\\推箱子素材\\esc_2.jpg");
	loadimage(&img_guanqia1, "\\推箱子素材\\guanqia_1.jpg");
	loadimage(&img_guanqia2, "\\推箱子素材\\guanqia_2.jpg");
	//铺地板背景图 
	for (i = 1; i <= WIDTH / SIZE + 1; i++)
	{
		for (j = 1; j <= HIGH / 2 / SIZE + 1; j++)
...
void View3()
{
	...
	loadimage(&img_backgroundbrick, "\\推箱子素材\\backgroundbrick.jpg", size / 2, size);
	loadimage(&img_man1, "\\推箱子素材\\man_1.jpg", size, size);
	loadimage(&img_man2, "\\推箱子素材\\man_2.jpg", size, size);
	loadimage(&img_menu1, "\\推箱子素材\\menu_1.jpg", 3 * SIZE, 3 * SIZE * 128 / 268);
	loadimage(&img_menu2, "\\推箱子素材\\menu_2.jpg", 3 * SIZE, 3 * SIZE * 128 / 268);
	loadimage(&img_brick, "\\推箱子素材\\brick.jpg", size, size);
	loadimage(&img_aim1, "\\推箱子素材\\aim_1.jpg", size, size);
	loadimage(&img_aim2, "\\推箱子素材\\aim_2.jpg", size, size);
	loadimage(&img_box, "\\推箱子素材\\box.jpg", size, size);
	loadimage(&img_successbox, "\\推箱子素材\\successbox.jpg", size, size);
	loadimage(&img_up, "\\推箱子素材\\up.jpg", size, size);
	loadimage(&img_down, "\\推箱子素材\\down.jpg", size, size);
	loadimage(&img_left, "\\推箱子素材\\left.jpg", size, size);
	loadimage(&img_right, "\\推箱子素材\\right.jpg", size, size);
	//绘制地板背景图
	cleardevice();
	...
	outtextxy(1050, 110, "上:键盘W/w/↑/8/鼠标点击上标");
	outtextxy(1050, 170, "下:键盘S/s/↓/2/鼠标点击下标");
	outtextxy(1050, 230, "左:键盘A/a/←/4/鼠标点击左标");
	outtextxy(1050, 290, "右:键盘D/d/→/6/鼠标点击右标");
...
void Updatewithinput()
{
	...
	case 3:
	{
		...
		if (_kbhit())
		{
			...
			input1 = _getch();
			if (input1 == 56 || input1 == 87 || input1 == 119 || input1 == 72)
			{
				movechoice = 1;
				movetype = Move(movechoice);
			}
			if (input1 == 50 || input1 == 83 || input1 == 115 || input1 == 80)
			{
				movechoice = 2;
				movetype = Move(movechoice);
			}
			if (input1 == 52 || input1 == 65 || input1 == 97 || input1 == 75)
			{
				movechoice = 3;
				movetype = Move(movechoice);
			}
			if (input1 == 54 || input1 == 68 || input1 == 100 || input1 == 77)
			{
				movechoice = 4;
				movetype = Move(movechoice);
			}
